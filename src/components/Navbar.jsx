...
import { Menu, X, ChevronDown, Bell, User, LogOut, Settings, HelpCircle } from 'lucide-react';
...
const Navbar = ({ scrolled }) => {
    ...
    return (
        <nav className={`premium-navbar ${scrolled ? 'scrolled' : ''} ${isOpen ? 'menu-open' : ''}`}>
            <div className="navbar-wrapper">
                ...
                <ul className="nav-links">
                    {menuItems.map((item) => (
                        ...
                        >
                            <Link to={item.path} className={`nav-link ${location.pathname === item.path ? 'current' : ''}`}>
                                {item.name}
                ...
                <div className="nav-actions">
                    <div className="nav-utils">
                        ...
                        <div className="util-links">
                            <Link to="/faq" className="util-link">{t('faq')}</Link>
                        </div>
                    </div>
                    <a
                    ...
                    {menuItems.map((item) => (
                        <li key={item.name} className="sidebar-item">
                            <div className="sidebar-link-group">
                                <Link to={item.path} className="sidebar-link">{item.name}</Link>
                                {item.dropdown && (
                                    <button
                            ...
                            {item.dropdown && (
                                <ul className={`sidebar-dropdown ${activeDropdown === item.name ? 'show' : ''}`}>
                                    {item.dropdown.map((sub, i) => (
                                        <li key={i}><Link to={sub.path}>{sub.name}</Link></li>
                                    ))}
                                </ul>
                            )}
                        </li>
