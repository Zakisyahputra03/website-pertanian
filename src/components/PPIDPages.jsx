...
const PPIDLayout = ({ title, children }) => {
  ...
  const navItems = [
    ...
    {
      name: "Tugas Fungsi PPID",
      path: "/ppid/tugas-fungsi",
        ...
        <div className="ppid-main-grid">
          <aside className="ppid-sidebar reveal">
            ...
            >
              ...
              >
                "Keterbukaan Informasi adalah Kunci Pelayanan Publik yang
                Melayani"
              </p>
            </div>
          </aside>

          <main className="ppid-content-card reveal">{children}</main>
        </div>
      </div>
    </div>
  );
...
export const ProfilPPID = () => {
  ...
  useEffect(() => {
    const fetchPPIDData = async () => {
      try {
        setLoading(true);
        const result = await ApiService.getPPIDInstansi();
        // API PPID mengembalikan struktur: { response: "1", result: [...] }
        setInstansiData(Array.isArray(result) ? result : result.result || []);
      } catch (err) {
        setError(err.message);
        console.error("Error fetching PPID instansi:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <PPIDLayout title="Profil PPID">
      <p className="ppid-intro-text">
        PPID (Pejabat Pengelola Informasi dan Dokumentasi) Dinas Pertanian
        Provinsi Sumatera Barat adalah unit kerja yang bertanggung jawab dalam
      ...
      <div className="transparency-grid">
        <div className="transparency-card">
          <div className="category-icon" style={{ background: "#ecfdf5" }}>
          ...
          <p style={{ fontSize: "0.95rem", color: "#64748b" }}>
            ...
            kebijakan publik dan program keputusan publik.
          </p>
        </div>
        <div className="transparency-card">
          <div className="category-icon" style={{ background: "#fef3c7" }}>
            <Scale color="#f59e0b" />
          </div>
          <h3 style={{ marginTop: "1.5rem", marginBottom: "1rem" }}>
            Landasan Hukum
          ...
          <p style={{ fontSize: "0.95rem", color: "#64748b" }}>
            Diatur melalui Peraturan Gubernur Sumatera Barat tentang Pedoman
            Pengelolaan Pelayanan Informasi dan Dokumentasi.
          </p>
        </div>
      </div>

      {/* Display PPID Instansi Data */}
      {instansiData.length > 0 && (
        <div className="ppid-instansi-section">
          <h3>Data Instansi PPID</h3>
          <div className="instansi-list">
            {instansiData.map((instansi, index) => (
              <div key={instansi.id || index} className="instansi-card">
                <h4>
                  {instansi.nama || instansi.name || `Instansi ${index + 1}`}
                </h4>
                <p>
                  {instansi.deskripsi ||
  ...
  <PPIDLayout title="Visi & Misi PPID">
    ...
    >
      ...
      <h2 style={{ fontSize: "2.2rem", margin: "1rem 0", lineHeight: "1.3" }}>
        "Terwujudnya Pelayanan Informasi Publik yang Cepat, Tepat, dan
        Transparan Menuju Sumbar Madani."
      </h2>
    </div>
      ...
      {[
        {
          ...
          desc: "Meningkatkan kualitas pelayanan informasi publik dengan memanfaatkan teknologi informasi terbaru.",
        },
        {
          title: "Kompetensi SDM",
          desc: "Meningkatkan kompetensi pengelola PPID dalam melayani permintaan rincian teknis informasi pertanian.",
        },
        {
          title: "Standardisasi",
          desc: "Membangun sistem pendokumentasian informasi publik yang terintegrasi dan mudah diakses.",
        },
        {
          title: "Advokasi Informasi",
          desc: "Memberikan edukasi kepada masyarakat tani mengenai hak-hak atas informasi pembangunan daerah.",
        },
      ].map((step, i) => (
...
export const TugasFungsiPPID = () => (
  <PPIDLayout title="Tugas & Fungsi PPID">
    <p className="ppid-intro-text">
      PPID Pembantu Dinas Pertanian Sumbar memiliki peran krusial dalam
      menjembatani kebutuhan data masyarakat dengan kebijakan internal
      kedinasan.
    </p>
    ...
    <div className="transparency-grid">
      <div className="transparency-card">
        ...
        >
          <li>
            Mengkoordinasikan pengumpulan data informasi publik dari setiap
            bidang.
          </li>
    ...
    <div
      ...
      style={{ marginTop: "4rem", borderColor: "#3b82f6" }}
    >
      <div
      ...
      <div className="support-text">
        ...
        <p>
          Masyarakat berhak mengajukan keberatan jika permintaan informasi tidak
          ditanggapi dalam waktu 10+7 hari kerja.
        </p>
      </div>
    </div>
  </PPIDLayout>
...
export const InformasiPublik = () => {
  ...
  return (
    <PPIDLayout title="Daftar Informasi Publik">
      <p className="ppid-intro-text">
        Unduh dokumen dan data publik yang disediakan oleh Dinas Pertanian
        Provinsi Sumatera Barat sesuai kategori yang telah ditetapkan.
      </p>
      ...
      <div
        ...
        style={{ marginTop: "5rem", background: "#f8fafc" }}
      >
        <Link
          to="/agenda"
